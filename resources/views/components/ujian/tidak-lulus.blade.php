        ...
        <div class="col-sm-12">
            <div class="card">
                ...
                <div class="card-body text-justify">
                    <p>
                        Mohon maaf, Anda tidak lulus ujian ini.
                        ...
                        <div class="mb-3">
                            @if($ujian->ujian->end_date > now())
                                Silahkan mengikuti ujian kembali.
                            @else
                                Kuota yang tersedia untuk mengikuti ujian ini sudah penuh. Silahkan mengikuti ujian kembali pada waktu yang akan datang.
                            @endif
                        </div>

                        <div class="mb-3">
                            <a href="{{ route('simulasi.ujian.cetak', $ujian->id) }}" class="btn btn-primary" target="_blank">
                        ...
                        @if(!$ujian->ujian->pembahasan)
                            ...
                            <div class="mb-3">
                                <b>Atau lakukan pembayaran sebesar Rp. {{ number_format($ujian->ujian->harga, 0, ',', '.') }} untuk mendapatkan akses pembahasan soal.</b>
                                <button type="button" class="btn btn-warning mt-2 bayar"> <i class="fas fa-money-bill-wave"></i> Bayar Sekarang</button>
                            </div>
                        @endif
...
<script>
    $(function () {
        ...
        $(document).on('click', '.bayar', function () {
            $.ajax({
                url: "{{ route('simulasi.ujian.pembayaran', $ujian->id) }}",
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                    ujian_id: "{{ $ujian->ujian_id }}"
                ...
                success: function (response) {
                    if (response.success) {
                        //swall interval 1 seconds and then redirect
